...
class easyRelatedPostsPROAdmin {
	...
	public function saveOptions() {
		...
		if ( isset( $_POST ['dsplLayout'] ) ) {
			...
			if ( $theme ) {
				...
				$message = new WP_Error_Notice( 'Theme ' . $_POST ['dsplLayout'] . ' not found. Theme options discarded', 1 );
				WP_Admin_Notices::getInstance()->addNotice( $message );
			}
		}
		// Save the rest of the options
		$mainOptionsObj = new erpPROMainOpts();
		$result = $mainOptionsObj->saveOptions( $_POST );

		$message = $result
			? new WP_Updated_Notice( 'Options saved', 1 )
			: new WP_Error_Notice( 'No changes detected, options not updated', 1 );
	...
	/**
	 * Clears cache.
	 * !IMPORTANT! Not to be called directly. Only through ajax
	 *
	 * @author Panagiotis Vagenas <pan.vagenas@gmail.com>
	 * @since 1.0.0
	 */
	...
	public function killLic() {
		...
		if ( isset( $_POST['erp_license_deactivate'] ) ) {
			...
			if ( ! check_admin_referer( 'erp_nonce', 'erp_nonce' ) ) {
				...
				// Call the custom API.
				$response = wp_remote_get( add_query_arg( $api_params, EDD_SL_ERP_PRO_STORE_URL ), array( 'timeout'   => 15,
				                                                                                          'sslverify' => false
					) );
